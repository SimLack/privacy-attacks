...
class DynSDNE(DynamicGraphEmbedding):
    ...
    def learn_embedding(self, graph=None, edge_f=None,
        ...
        """
        x_diff1 = Lambda(lambda ab: ab[0] - ab[1],lambda L: L[1])([x_hat1, x1])
        """
        x_diff2 = merge([x_hat2, x2],
                        mode=lambda ab: ab[0] - ab[1],
                        output_shape=lambda L: L[1])
        """
        x_diff2 = Lambda(lambda ab: ab[0] - ab[1],lambda L: L[1])([x_hat2, x2])
        """
        y_diff = merge([y2, y1],
                       mode=lambda ab: ab[0] - ab[1],
                       output_shape=lambda L: L[1])
        """
        y_diff = Lambda(lambda ab: ab[0] - ab[1],lambda L: L[1])([y2,y1])
