...
## for sanity check:
sys.path.insert(0, str(os.getcwd())+"/../../dySat/")
...
class SDNE(DynamicGraphEmbedding):
    ...
    def learn_embedding(self, graph, save_info,prevStepInfo=True, loadfilesuffix=None, savefilesuffix=None, subsample=False):
        if subsample:
        ...
        print("prevstepinfo is:",prevStepInfo)
        if(prevStepInfo):
            ...
            if loadfilesuffix is None:
                ...
                encoder_prev = loadmodel(save_info.BASE_PATH+'/temp_weights/encoder_model_'+loadfilesuffix+'.json')
                decoder_prev = loadmodel(save_info.BASE_PATH+'/temp_weights/decoder_model_'+loadfilesuffix+'.json')
                loadweights(encoder_prev, save_info.BASE_PATH+'/temp_weights/encoder_weights_'+loadfilesuffix+'.hdf5')
                loadweights(decoder_prev, save_info.BASE_PATH+'/temp_weights/decoder_weights_'+loadfilesuffix+'.hdf5')
            ...
            if(self._node_num > prev_node_num):
                ...
                self._encoder = get_encoder(self._node_num, n_units[-1], self._n_units, self._nu1, self._nu2, self._actfn)
                self._decoder = get_decoder(self._node_num, n_units[-1], self._n_units, self._nu1, self._nu2, self._actfn)
        ...
        def weighted_mse_y(y_true, y_pred):
            ...
            return KBack.sum(KBack.square(y_pred), axis=-1) * y_true

            #return T.reshape(tensor=(KBack.sum(KBack.square(y_pred), axis=-1)),newshape=[min_batch_size, 1]) * y_true
        ...
        if(savefilesuffix is not None):
            saveweights(self._encoder, save_info.BASE_PATH+'/temp_weights/encoder_weights_'+savefilesuffix+'.hdf5')
            saveweights(self._decoder, save_info.BASE_PATH+'/temp_weights/decoder_weights_'+savefilesuffix+'.hdf5')
            savemodel(self._encoder, save_info.BASE_PATH+'/temp_weights/encoder_model_'+savefilesuffix+'.json')
            savemodel(self._decoder, save_info.BASE_PATH+'/temp_weights/decoder_model_'+savefilesuffix+'.json')
            # Save the embedding
            np.savetxt(save_info.BASE_PATH+'/temp_weights/embedding_'+savefilesuffix+'.txt', self._Y)
        ...
        val_results, test_results, _, _ = evaluate_classifier(train_edges, train_edges_false, val_edges,val_edges_false, test_edges, test_edges_false, self._Y, self._Y)
        print(">>> val train <<< :",val_results['HAD'][0])
        print(">>> test train <<< :",test_results['HAD'][0])
        #"""
        del self._model, sgd, self._autoencoder
    ...
    def get_embedding(self, filesuffix):
        return self._Y if filesuffix is None else np.loadtxt(save_info.BASE_PATH+'/temp_weights/embedding_'+filesuffix+'.txt')
    ...
    def get_edge_weight(self, i, j, embed=None, filesuffix=None):
        if embed is None:
            if filesuffix is None:
                ...
                embed = np.loadtxt(save_info.BASE_PATH+'/temp_weights/embedding_'+filesuffix+'.txt')
        if i == j:
            return 0
        else:
