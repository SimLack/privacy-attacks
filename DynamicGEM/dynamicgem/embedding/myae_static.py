...
import sys
#from embAttack.embedding-attack.graphs.graph_class import Graph as gc
...
class AE(StaticGraphEmbedding):
    ...
    def learn_embeddings(self, graph=None, edge_f=None):
        # turn Graph into nx_graph
        if graph != None:
            nx_graph = gc.to_networkx(graph) 
            print("nx_GRAPHHHHHHHHHHHH")
            print(nx_graph) 
        ...
        if self._savefilesuffix is not None:
            saveweights(self._encoder,
                        "output/sbm_cd/staticAE/" + 'encoder_weights_' + self._savefilesuffix + '.hdf5')
            saveweights(self._decoder,
                        "output/sbm_cd/staticAE/" + 'decoder_weights_' + self._savefilesuffix + '.hdf5')
            savemodel(self._encoder,
                       "output/sbm_cd/staticAE/" + 'encoder_model_' + self._savefilesuffix + '.json')
            savemodel(self._decoder,
                       "output/sbm_cd/staticAE/" + 'decoder_model_' + self._savefilesuffix + '.json')
            # Save the embedding
            np.savetxt("output/sbm_cd/staticAE/" + 'embedding_' + self._savefilesuffix + '.txt',
                       self._Y)
