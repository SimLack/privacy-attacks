...
if FLAGS.graph1 == "":
    ...
    graphs, adjs = load_graphs(FLAGS.dataset,graph1=FLAGS.graph1)
else:
    graphs, adjs = load_graphs(FLAGS.dataset,graph1=FLAGS.graph1,graph2=FLAGS.graph2)
...
for epoch in range(FLAGS.epochs):
    ...
    if (epoch + 1) % FLAGS.test_freq == 0:
        ...
        if FLAGS.window < 0:
            #print("flags.window is <0")
            #print(FLAGS.time_steps,model.final_output_embeddings.get_shape()[1])
            assert FLAGS.time_steps == model.final_output_embeddings.get_shape()[1]
        #print("flags timesteps:",FLAGS.time_steps)
        #print("model final output embeddings")
        #print(model.final_output_embeddings.get_shape())
        print("epoch is:",epoch,"FLAGS.epochs-1 is:",FLAGS.epochs-1)
        """
        if epoch == FLAGS.epochs-1:
            print("finaloutputembedding")
            print(np.shape(sess.run(model.final_output_embeddings, feed_dict=feed_dict)))
            print((sess.run(model.final_output_embeddings, feed_dict=feed_dict)))
            print("finaloutputembedding nur letzter")
            print(np.shape(sess.run(model.final_output_embeddings, feed_dict=feed_dict)[:,FLAGS.time_steps-1,:]))

            print((sess.run(model.final_output_embeddings, feed_dict=feed_dict)[:,FLAGS.time_steps-1,:]))
        """
        emb = sess.run(model.final_output_embeddings, feed_dict=feed_dict)[:,
              model.final_output_embeddings.get_shape()[1] - 2, :] #usually -2
        emb = np.array(emb)
        #print("EMBBBBBBBBBBBBBBBB")
        #print((emb))
...
if FLAGS.graph2!="": 
    np.save(save_emb_dir+"/_1_"+FLAGS.graph1+"_2_"+FLAGS.graph2,emb)
elif FLAGS.graph1!="":
    np.save(save_emb_dir+"/_1_"+FLAGS.graph1,emb)
