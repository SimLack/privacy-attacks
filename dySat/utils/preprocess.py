...
def load_graphs(dataset_str,graph1:str=None,graph2:str=None):
    ...
    #"""
    #x = np.load("data/{}/{}".format(dataset_str, "graphs.npz"))
    #x1 = np.load("data/{}/{}".format(dataset_str, "graphs1.npz"), allow_pickle=True)
    #list(x.items())
    if graph1==None:
        graph = nx.Graph()
        graph.add_nodes_from(np.load("data/{}/{}".format(dataset_str, "graphs.npz"), allow_pickle=True)['nodes'])
        graph.add_edges_from(np.load("data/{}/{}".format(dataset_str, "graphs.npz"), allow_pickle=True)['edges'])
        graph1 = nx.Graph()
        graph1.add_nodes_from(np.load("data/{}/{}".format(dataset_str, "graphs1.npz"), allow_pickle=True)['nodes'])
        graph1.add_edges_from(np.load("data/{}/{}".format(dataset_str, "graphs1.npz"), allow_pickle=True)['edges'])
        graphs = [graph,graph,graph1]
    elif graph2==None:
        firstGraph = nx.Graph()
        firstGraph.add_nodes_from(np.load(FLAGS.save_graphs_embs_in+"temp_graphs/"+graph1.replace(", ","x")+".npz", allow_pickle=True)['nodes'])
        firstGraph.add_edges_from(np.load(FLAGS.save_graphs_embs_in+"temp_graphs/"+graph1.replace(", ","x")+".npz", allow_pickle=True)['edges'])
        graphs = [firstGraph,firstGraph]
    else:
        firstGraph = nx.Graph()
        firstGraph.add_nodes_from(np.load(FLAGS.save_graphs_embs_in+"temp_graphs/"+graph1.replace(", ","x")+".npz", allow_pickle=True)['nodes'])
        firstGraph.add_edges_from(np.load(FLAGS.save_graphs_embs_in+"temp_graphs/"+graph1.replace(", ","x")+".npz", allow_pickle=True)['edges'])
        secondGraph = nx.Graph()
        secondGraph.add_nodes_from(np.load(FLAGS.save_graphs_embs_in+"temp_graphs/"+graph2.replace(", ","x")+".npz", allow_pickle=True)['nodes'])
        secondGraph.add_edges_from(np.load(FLAGS.save_graphs_embs_in+"temp_graphs/"+graph2.replace(", ","x")+".npz", allow_pickle=True)['edges'])
        graphs = [firstGraph,firstGraph,secondGraph]
...
def get_evaluation_data(adjs, num_time_steps, dataset):
    ...
    train_edges, train_edges_false, val_edges, val_edges_false, test_edges, test_edges_false = \
        create_data_splits(adjs[eval_idx], next_adjs, val_mask_fraction=0.02, test_mask_fraction=0.18)
