...
if FLAGS.graph1 == "":
    ...
    print("\ngraph0:",len(graphs[0].edges()),list(graphs[0].edges())[:50])
else:
    graphs, adjs = load_graphs(FLAGS.dataset,graph1=FLAGS.graph1,graph2=FLAGS.graph2)
    print("\ngraph0:",len(graphs[0].edges()),list(graphs[0].edges())[:50])
    print("\ngraph1:",len(graphs[2].edges()),list(graphs[2].edges())[:50])
...
num_time_steps = FLAGS.time_steps
print("FLAGS.timesteps is:",FLAGS.time_steps)
...
prev_hidden_embeds = []
if num_time_steps > 2:  # in case of 2, nothing to be done.  ##usually 2
...
for epoch in range(FLAGS.epochs):
    ...
    if epoch % FLAGS.test_freq == 0:
        ...
        if (epoch == 0) or (epoch > 0 and epoch_auc_val >= max(epochs_val_result["HAD"])):
            ...
            print("modeldir:",MODEL_DIR,"flags dataset:",FLAGS.dataset)
...
print("max value:",max(epochs_val_result["HAD"]))
...
save_emb_dir = FLAGS.save_graphs_embs_in+"temp_embeddings"
print("save_emb_dir is:",save_emb_dir)

if FLAGS.graph2!="": #change for link prediction
