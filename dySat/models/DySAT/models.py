...
class DySAT(Model):
    ...
    def __init__(self, placeholders, num_features, num_features_nonzero, degrees, **kwargs):
        ...
        print("struc layer conf")
        print(self.structural_layer_config)
        self.structural_layer_config = list(map(int, FLAGS.structural_layer_config.split(",")))
        print(FLAGS.structural_layer_config)
        print(self.structural_layer_config)
    ...
    def build_net(self, attn_head_config, attn_layer_config, temporal_head_config, temporal_layer_config,
        ...
        for i in range(0, len(list(attn_layer_config))):
            print("i:",i,"/",len(list(attn_layer_config)))
            if i > 0:
                input_dim = attn_layer_config[i - 1]
                sparse_inputs = False
            self.structural_attention_layers.append(StructuralAttentionLayer(input_dim=input_dim,
                                                                             output_dim=list(attn_layer_config)[i],
                                                                             n_heads=list(attn_head_config)[i],
