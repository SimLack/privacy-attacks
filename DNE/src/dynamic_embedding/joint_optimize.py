...
class NodeEmbedding(object):
    def __init__(self, params, init_embeddings, init_weights, G, mapp, rmapp, num_new = 1):
        ...
        with self.tensor_graph.as_default():
            ...
            self.constant_embeddings = tf.constant(init_embeddings[: self.num_nodes_init - self.num_modify],dtype = tf.float32)
            self.constant_weights = tf.constant(init_weights[: self.num_nodes_init - self.num_modify],dtype=tf.float32)
            self.modify_embeddings_pre = tf.constant(init_embeddings[self.num_nodes_init - self.num_modify :],dtype=tf.float32)
            self.modify_weights_pre = tf.constant(init_weights[self.num_nodes_init - self.num_modify :],dtype=tf.float32)
            ...
            #print("embedddddings float")
            #print(type(self.constant_embeddings))
            #print(type(self.modify_embeddings))
            #print(type(self.new_embeddings))
